/************************************************************************
 *  Typewriting Demo on Dual LCD (2004 + 1602) using 4x4 Matrix Keypad  *
 ************************************************************************
 * 
 * Description:
 ...
 *   In this example Arduino digital pins D4, D5, D6, and D7 are used in common with both
 *   LCD databus pins and also with the matrix keypad column pins.
 *   Both LCD using the same digital pins D2 and D3 to drive RS and RW pins respectively.
 ...
 *   |        RS |  ----  D2
 *   |        RW |  ----  D3
 *   |        EN |  ----  D8/A2 (D8 to LCD2004, A2 to LCD1602)
 *   |       DB0 |
 *   |       DB1 |        A1  ----  [ Buzzer ]  ---------------  GND
 *   |       DB2 |        A0  ----  [ 4k7Ω R ] ----
 *   |       DB3 |                                 |
...
LCD_HD44780 lcd1(LCD_RS, LCD_RW, LCD_1_EN, LCD_D4, LCD_D5, LCD_D6, LCD_D7);
LCD_HD44780 lcd2(LCD_RS, LCD_RW, LCD_2_EN, LCD_D4, LCD_D5, LCD_D6, LCD_D7);
LCD_HD44780 *lcd = &lcd1;
LCD_HD44780 *otherLcd = &lcd2;
...
void setup()
{
  ...
  createCustomChars( &lcd2 );
  lcd1.print( F( "Multitap Keypad\nby ZulNs" ) );
  lcd2.print( F( "Multitap Keypad\nby ZulNs" ) );
  delay( 5000 );
  lcd1.print( F( "\nPress [*] key..." ) );
  lcd2.print( F( "\nPress [*] key..." ) );
      ...
      switch ( key.code )
      {
        ...
        case KEY_C:
          ...
          else if ( !isEditMode )
          {
            ...
            initFlashing();
            lcd->write( UPPER_D );
            lcd->write( isLCD1 ? NUMBER_1 : NUMBER_2 );
            lcd->setCursor( cursorPos );
            lcd->gotoCursor();
          }
          break;
        case KEY_D:
          ...
          else if ( !isEditMode )
          {
            ...
            lcd->setCursor( cursorPos );
            lcd->gotoCursor();
            cursorPos = otherLcd->getCursor();
            otherLcd->clearLine();
            otherLcd->print( copyBuf );
            otherLcd->setCursor( cursorPos );
          }
          break;
        default:
          if ( isAlphaMode )
...
void createCustomChars( LCD_HD44780 * pLCD )
{
  pLCD->createChar( UPPER_A,  letter_A );
  pLCD->createChar( UPPER_B,  letter_B );
  pLCD->createChar( LOWER_A,  letter_a );
  pLCD->createChar( LOWER_B,  letter_b );
  pLCD->createChar( NUMBER_1, number_1 );
  pLCD->createChar( NUMBER_2, number_2 );
  pLCD->createChar( UPPER_E,  letter_E );
  pLCD->createChar( UPPER_D,  letter_D );
}
